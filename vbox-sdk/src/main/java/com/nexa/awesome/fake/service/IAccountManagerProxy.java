...
import com.nexa.awesome.fake.frameworks.BAccountManager;
...
import com.nexa.awesome.utils.auth.RealAccountTokenFetcher;
...
public class IAccountManagerProxy extends BinderInvocationStub {
    ...
    public static class getAuthToken extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (tokenType.contains("google")) {
                String token = TokenCache.getGoogleToken();
                if (token == null) {
                    token = RealAccountTokenFetcher.fetch(NexaCore.getContext(), account, tokenType);
                    TokenCache.setGoogleToken(token);
                }
                if (token != null) {
                    Bundle bundle = new Bundle();
                    bundle.putString("authtoken", token);
                    ((android.accounts.IAccountManagerResponse) args[0]).onResult(bundle);
                    return 0;
                }
            } else if (tokenType.contains("facebook")) {
                String token = TokenCache.getFacebookToken();
                if (token != null) {
                    Bundle bundle = new Bundle();
                    bundle.putString("authtoken", token);
                    ((android.accounts.IAccountManagerResponse) args[0]).onResult(bundle);
            ...
            } else if (tokenType.contains("twitter")) {
                String token = TokenCache.getTwitterToken();
                if (token != null) {
                    Bundle bundle = new Bundle();
                    bundle.putString("authtoken", token);
                    ((android.accounts.IAccountManagerResponse) args[0]).onResult(bundle);
            ...
            // 👉 fallback to virtual manager
            return hook(who, method, args);
        }
    }
