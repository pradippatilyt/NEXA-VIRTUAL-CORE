...
import com.nexa.awesome.fake.hook.IInjectHook;
...
public class HCallbackStub implements IInjectHook, Handler.Callback {
    ...
    private Handler getH() {
        Object currentActivityThread = NexaCore.mainThread();
    ...
    private Boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                NexaCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = NexaCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            NexaCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(NexaCore.mainThread()).getLaunchingActivity(token);
                if (record != null) {
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid())) && !serviceInfo.name.equals(ProxyManifest.getProxyJobService(BActivityThread.getAppPid()))) {
                Slog.d(TAG, "handleCreateService: " + data);
                Intent intent = new Intent();
                intent.setComponent(new ComponentName(appPackageName, serviceInfo.name));
                NexaCore.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
