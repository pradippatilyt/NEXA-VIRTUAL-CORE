...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        try {
            ...
            splash.setClass(NexaCore.getContext(), LauncherActivity.class);
            ...
            NexaCore.getContext().startActivity(splash);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        try {
            ...
            // ✅ FIX: Prevent null extras
            if (launchIntent.getExtras() == null) {
               launchIntent.replaceExtras(new Bundle());
            }
            ...
            if (nameView != null) {
                ...
                nameView.animate().alpha(1f).setDuration(500).setStartDelay(200).start();
            }
            if (iconView != null && drawable != null) {
                ...
                iconView.animate().scaleX(1.1f).scaleY(1.1f).alpha(1f).setDuration(350).setInterpolator(new OvershootInterpolator()).withEndAction(() -> iconView.animate().scaleX(1f).scaleY(1f).setDuration(150).setInterpolator(new android.view.animation.DecelerateInterpolator()).start()).start();
            }
    ...
    private PackageInfo getPackageInfoWithFallback(String packageName, int userId) {
        try {
            return NexaCore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        } catch (Exception e) {
            Slog.w(TAG, "Failed to get package info for " + packageName + " (attempt 1): " + e.getMessage());
            try {
                return NexaCore.getBPackageManager().getPackageInfo(packageName, 
                ...
                try {
                    android.content.pm.ApplicationInfo appInfo = NexaCore.getBPackageManager().getApplicationInfo(packageName, 0, userId);
                    if (appInfo != null) {
    ...
    private void launchAppAsync(final Intent launchIntent, final int userId) {
        new Thread(() -> {
            try {
                ...
                Thread.sleep(100);
                NexaCore.getBActivityManager().startActivity(launchIntent, userId);
