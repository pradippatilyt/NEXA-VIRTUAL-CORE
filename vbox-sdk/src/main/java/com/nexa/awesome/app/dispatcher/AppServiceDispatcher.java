package com.nexa.awesome.app.dispatcher;
...
import com.nexa.awesome.entity.ServiceRecord;
import com.nexa.awesome.entity.UnbindRecord;
import com.nexa.awesome.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = NexaCore.get().getHandler();
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            NexaCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = NexaCore.getBActivityManager().onServiceUnbind(proxyIntent, BActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    NexaCore.getBActivityManager().onServiceDestroy(proxyIntent, BActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    NexaCore.getBActivityManager().onServiceDestroy(intent, BActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
