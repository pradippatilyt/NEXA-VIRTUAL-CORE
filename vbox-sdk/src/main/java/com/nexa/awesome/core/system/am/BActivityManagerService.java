...
public class BActivityManagerService extends IBActivityManagerService.Stub implements ISystemService {
    ...
    public Intent sendBroadcast(Intent intent, String resolvedType, int userId) throws RemoteException {
        List<ResolveInfo> resolves = BPackageManagerService.get().queryBroadcastReceivers(intent, FileUtils.FileMode.MODE_IWUSR, resolvedType, userId);
        ...
        shadow.setPackage(NexaCore.getHostPkg());
    ...
    @Override
    public void onActivityCreated(int taskId, IBinder token, String activityRecord) throws RemoteException {
        int callingPid = Binder.getCallingPid();
      ...
      //  ActivityRecord record = (ActivityRecord) activityRecord;
        ...
        synchronized (userSpace.mStack) {
            userSpace.mStack.onActivityCreated(process, taskId, token, activityRecord);
        }
    }
    ...
    public RunningAppProcessInfo getRunningAppProcesses(String callerPackage, int userId) throws RemoteException {
        ActivityManager manager = (ActivityManager)
                NexaCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    ...
    public void scheduleBroadcastReceiver(Intent intent, PendingResultData pendingResultData, int userId) throws RemoteException {
        List<ResolveInfo> resolves = BPackageManagerService.get().queryBroadcastReceivers(intent, FileUtils.FileMode.MODE_IWUSR, null, userId);
