...
//                    }
        ...
        try {
            base = NexaCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = NexaCore.getPackageManager().getApplicationInfo(NexaCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = NexaCore.getPackageManager().getPackageArchiveInfo(sourceDir, 0).applicationInfo;
        }
...
//            ApplicationInfo base = NexaCore.getContext().getApplicationInfo();
