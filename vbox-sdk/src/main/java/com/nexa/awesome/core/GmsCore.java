...
import com.nexa.awesome.utils.auth.Auth;
...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        ...
        for (String packageName : list) {
            if (nexaCore.isInstalled(packageName, userId)) {
                continue;
            }

            try {
                NexaCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException ignored) {
                continue;
            }

            InstallResult installResult = nexaCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        NexaCore nexaCore = NexaCore.get();
        for (String packageName : list) {
            nexaCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            NexaCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
        } catch (PackageManager.NameNotFoundException ignored) { }
        return false;
    }
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return NexaCore.get().isInstalled(GMS_PKG, userId);
    }
}